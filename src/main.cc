...
static int daemon_main(LiteClient& client, const bpo::variables_map& variables_map) {
  ...
  while (true) {
    ...
    auto cir = akclient.GetTargetToInstall(nullptr, -1, "", current, false);
    ...
    auto install_result =
        akclient.PullAndInstall(cir.selected_target, cir.reason, generate_correlation_id(cir.selected_target),
                                InstallMode::All, nullptr, true, true);
    if (akclient.RebootIfRequired(install_result)) {
...
static int daemon_main_original(LiteClient& client, const bpo::variables_map& variables_map) {
  FileLock lock;
  if (client.config.uptane.repo_server.empty()) {
    LOG_ERROR << "[uptane]/repo_server is not configured";
    return EXIT_FAILURE;
