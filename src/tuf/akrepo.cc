...
// AkRepo
AkRepo::AkRepo(boost::filesystem::path storage_path) { init(storage_path); }
...
void AkRepo::init(boost::filesystem::path storage_path) {
  StorageConfig sc;
...
// FetcherWrapper
AkRepo::FetcherWrapper::FetcherWrapper(std::shared_ptr<RepoSource> src) { repo_src = src; }
